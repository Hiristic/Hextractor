...
if __name__ == "__main__":
    ...
    #indexWriter.schema_get("imvelo-search-datasheets")
    indexWriter.schema_get("imvelo-search")
    #field_limit_doc = dict(list(fileparser.split_doc.items())[:54])

    print(len(fileparser.doc))
    field_limit_doc = dict(list(fileparser.doc.items())[:99])
    print(len(field_limit_doc))
    result = indexWriter.index_doc("imvelo-search-datasheets", field_limit_doc)
    print(result)
